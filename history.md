...
## 2026-09-24 — Reconsider grammatical pitch for nesting

Rubén recalled his proposal to express nesting levels through tonality. The earlier proposal is recorded under "Tonality side-question" on 2026-05-16. That entry rejected pitch partly because scope-by-prosody conflicted with the existing design, and asserted reliability problems without documenting a test of this particular encoding. The conflict identifies which decisions would change; it does not establish that a grammatical pitch system cannot work. Its "one syllable each" cost argument also does not describe the current pair: `bra` has one syllable and `keti` has two.

A11 now includes a candidate in which the rental is at a base pitch register, the nested truck phrase is at a higher register, and the modifier's register selects truck versus rental attachment. These are categorical grammatical levels relative to the speaker's baseline, not absolute musical notes or optional emotional intonation. If this encoding works, the transition itself adds zero syllables, saving three compared with an explicit `bra … keti` pair. That is a syllable-count prediction, not a measured speed or comprehension benefit.

Two candidates merit comparison: sustained relative pitch levels representing depth, or local pitch contours signalling entry and exit. Sustained levels expose current depth but require more distinct levels for deeper structures; local contours avoid assigning a separate sustained register to every depth but require the listener to retain the nesting state. Neither is a completed grammar. The small first test should contrast rental-level and truck-level modification with the same lexical material. Further checks must distinguish sibling groups at equal depth, recover closures, and separate nesting cues from emphasis. No listening test was run in this session.

Recommendation: evaluate grammatical pitch rather than dismiss it because the current spec says non-tonal. Keep explicit grouping as a candidate equivalent encoding when pitch cannot carry the distinction. A written pitch/boundary notation must represent the same structure; adoption would require revisiting the 2026-06-08 rule that grouping is always written and spoken as words. The trial remains open and no existing decision has been overwritten.

Rubén clarified the intended written/spoken split: keep the longer explicit written form, but use pitch to shorten speech. This resolves the candidate's writing question without inventing symbols: write `bra … keti`, realize the same boundaries tonally. The proposal revises the requirement to pronounce grouping words, not the existing choice of written grouping words. Fewer spoken syllables are predictable; faster delivery with equal comprehension remains a testable hypothesis. This clarification supersedes the preceding suggestion that a new written pitch/boundary notation is needed.

## 2026-09-24 — Separate requirements from design decisions

Rubén approved splitting the language's goals and constraints from the mechanisms selected to meet them. requirements.md now collects unambiguous meaning with permitted vagueness, optional information, efficient communication, clarity in difficult listening conditions, and recognizable vocabulary. The first two principles moved out of decisions.md; the remaining goals are traced to existing project descriptions and design rationales. Status notes distinguish active targets from demonstrated results, without adding numerical acceptance thresholds.

The split preserves word order, morphology, phoneme mapping, non-tonality, and grouping conventions as design decisions. A rule can be mandatory within the current grammar while remaining open to redesign in service of a requirement. In particular, recording grammatical pitch as an alternative does not adopt it. The alternative of simply renaming decisions.md was rejected because it would continue to mix desired outcomes with particular mechanisms.

README.md and AGENTS.md now distinguish the records, and the task board's line-number reference to the punctuation question was replaced with a topic reference. Existing historical entries and unrelated working-tree edits were preserved. This was a documentation reorganization, not a revision of Lexor's grammar or vocabulary.
