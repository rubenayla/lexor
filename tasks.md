<!-- the one task board for this repo. Completed work is in tasks/done-archive.md. -->
# Tasks

Open design questions parked here so we can address them one at a time.

**Done items do not stay on the board.** When a top-level item and every sub-question nested
under it are resolved, move the whole bullet — parent text plus its indented subtree, verbatim,
with its date and closing note — to `tasks/done-archive.md`, the only other task file and the one
that holds nothing actionable. Same convention as the partle and kart-medulla repos.

**Exception: a resolved top-level item with an open sub-question stays on the board whole.** A
design round often closes the main framework (`- [x]`) but leaves a follow-up parked (`- [ ]`)
underneath it — e.g. the quantifier framework is resolved but "many/few" fuzzy quantifiers are
still open. Archiving just the closed lines would strip the open sub-question of the context for
what was already settled, so the whole bullet — closed children included — stays put until the
last child closes too.
...
- [ ] **Rental offer vs truck modifier attachment** — work trial A11 in [trials.md](trials.md): formalize how a modifier targets a preceding constituent and how a closed group retains its head. Demonstrate rental-level versus truck-level attachment without relying on explanatory paraphrases. Separately define the absence relation so it cannot imply autonomous capability by context. Compare spoken grouping with grammatical pitch levels/entry-exit contours, keeping written `bra … keti` but expressing their boundaries through pitch in speech; check same-depth siblings and explicit pronunciation as an alternative. Revised grammar proposals: history.md, 2026-09-24.
...
- [x] **Negation** — resolved: prefix `no`, phonologically bound to its host. See decisions.md and history.md 2026-05-15. Related sub-questions parked:
    ...
    - [~] Canonical stacking order for multiple prefixes — first rule locked 2026-06-07: `no` is outermost, scoping over a `mi`/`ze` derived concept (`nomiX` = "not opposite-of-X"); `mi`/`ze` mutually exclusive on one root. Extend when interrogative/intensity prefixes are added. See decisions.md / history.md 2026-06-07.
    - [~] Reserve the prefix phonological inventory — current inventory locked at {`no`, `mi`, `ze`} (all CV, vowel-final, onto consonant-initial roots, exempt from subclass-vowel scheme). Reserve more shapes when new prefixes surface. See decisions.md 2026-06-07.
